...
//------------------------------------------------------------------------------
// Main method
void generate_HEPEvt_Decay_random(UInt_t ndecays, UInt_t nevents) {
  ...
  std::ofstream output_hepevt_larsoft(isotope + "_decays_HEPEvt_LArSoft_uniform.dat");
  ...
  if (isotope == "Ar41") {
	...
	// Generates position in protoDUNE (for each event, not each gamma)
	Xg.SetXYZ(rdef->Uniform(-350.,350.),rdef->Uniform(0.,600),rdef->Uniform(0.,700));
	//Xg.SetXYZ(-100, 400, 100);
      
      
	// Now the part where the HEPEvt and DANCE input files are written
	output_dance << nb_gammas << endl;
	output_hepevt_larsoft << e << " " << nb_gammas << endl;
